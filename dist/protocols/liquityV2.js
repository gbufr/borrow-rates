...
export class LiquityV2Scanner {
    ...
    async scanBranch(troveManagerAddress, collateralToken, blockNumber, timestamp) {
    ...
    async syncTroveV2(troveManager, userAddress, collateralAsset, blockNumber, timestamp) {
        try {
            ...
            try {
                const troveInfo = await publicClient.readContract({
                    address: troveManager,
                    abi: [
                        {
                            inputs: [{ name: '_borrower', type: 'address' }],
