...
export class MakerScanner {
    ...
    async getMarketRate(collateralAsset, debtAsset, marketId) {
        try {
            ...
            const ilkBytes = (ilkName.startsWith('0x') ? ilkName : '0x' + Buffer.from(ilkName).toString('hex').padEnd(64, '0'));
            const dutyData = await publicClient.readContract({
                address: JUG_ADDRESS,
    ...
    async syncMakerVault(cdpId, blockNumber, timestamp) {
        try {
