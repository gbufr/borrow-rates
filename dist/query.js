...
async function main() {
    ...
    if (protocol)
        console.log(`Filter Protocol: ${protocol}`);
    if (user)
        console.log(`Filter User: ${user}`);
    if (showAtRisk)
        console.log(`Filter: Show At-Risk Only (HF < 1.1)`);
    ...
    if (protocol) {
        filtered = filtered.filter((p) => p.protocol.toLowerCase() === protocol.toLowerCase());
    }
    if (user) {
        filtered = filtered.filter((p) => p.userAddress.toLowerCase() === user.toLowerCase());
    }
    if (minDebt > 0) {
        filtered = filtered.filter((p) => parseFloat(p.debtAmount) >= minDebt);
    }
    if (showAtRisk) {
        filtered = filtered.filter((p) => p.calculatedHF < 1.1);
    }
    // Slice results
