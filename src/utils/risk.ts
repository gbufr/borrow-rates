...
export const PROTOCOL_THRESHOLDS: Record<string, number> = {
  'AaveV3': 0.825, 
  'MorphoBlue': 0.90, 
  'LiquityV1': 0.909, 
  'LiquityV2': 0.909, 
...
export class RiskService {
  ...
  private db: ILoanRepository | null = null;

  constructor(db?: ILoanRepository) {
    if (db) this.db = db;
  }
  ...
  getSymbolForAsset(assetAddress: string): string {
    ...
    if (MAPPINGS[addr]) return MAPPINGS[addr];

    if (addr.includes('0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48')) return 'USDC';
    if (addr.includes('0x6b175474e89094c44da98b954eedeac495271d0f')) return 'DAI';
    if (addr.includes('0x7f39c581f595b53c5cb19bd0b3f8da6c935e2ca0')) return 'wstETH';
    if (addr.includes('0xcd5fe23c85820f7b72d0926fc9b05b43e359b7ee')) return 'weETH';
    if (addr.includes('0x5f98805a4e8be255a32880fdec7f6728c6568ba0')) return 'LUSD';
    if (addr.includes('0xdc035d45d973e3ec169d2276ddab16f1e407384f')) return 'USDS';

    if (assetAddress.length < 10) return assetAddress;
    return assetAddress.slice(0, 6) + '...' + assetAddress.slice(-4);
  }

  getPriceForAsset(assetAddress: string, symbolHint?: string): number {
    if (symbolHint) {
  ...
  getPriceForSymbol(symbol: string): number {
    ...
    if (s === 'ETH' || s === 'WETH') return this.priceCache['ETH'] || 2500;
    if (s === 'WBTC' || s === 'CBBTC') return this.priceCache['WBTC'] || 65000;
    if (s === 'POL' || s === 'WPOL' || s === 'MATIC') return this.priceCache['POL'] || 0.45;
    if (s === 'JPYC' || s === 'MJPYC') return this.priceCache['JPYC'] || this.priceCache['MJPYC'] || 0.0066;
    if (s === 'MKGLD') return this.priceCache['MKGLD'] || 82.0;
    if (s === 'USR') return this.priceCache['USR'] || 0.18;
    if (s === 'BONDUSD') return this.priceCache['BONDUSD'] || 0.21;
    if (s === 'WSTETH' || s === 'WEETH' || s === 'STETH') return (this.priceCache['ETH'] || 2500) * 1.25;

    const stablecoins = ['USDC', 'DAI', 'USDS', 'BOLD', 'LUSD', 'USDT', 'RLUSD'];
    if (stablecoins.includes(s)) return 1.0;
