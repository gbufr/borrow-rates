    ...
    for (const pair of PAIRS) {
      try {
        ...
        if (protocolName.includes('Morpho Blue')) {
          ...
          if (pair.debt.symbol !== 'DAI') continue; // Maker only lends DAI
          if (pair.coll.symbol === 'WETH') marketId = 'ETH-A';
          else if (pair.coll.symbol === 'WBTC') marketId = 'WBTC-A';
          else if (pair.coll.symbol === 'wstETH') marketId = 'WSTETH-A';
          else continue;
          ...
          continue;
           ...
           if (pair.coll.symbol !== 'WETH' || pair.debt.symbol !== 'LUSD') continue;
        } else if (protocolName.includes('Sky')) {
           if (pair.debt.symbol !== 'USDS') continue;
        ...
        if (rate !== null) {
          ...
          let liqThreshold: number | null = null;
          let liqPenalty: number | null = null;
          ...
          await db.upsertRate({
            ...
            chain: (scanner as any).chain || 'Ethereum',
